...
const APP_STORE_URL = "https://apps.apple.com/us/app/proteinsnaps/id6801353318";
...
export function HeroSection() {
  ...
  const startAutoPlay = useCallback(() => {
    clearAutoPlay();
    // Desktop hero is manual-only — no auto-advance of images/text
    if (isDesktop) return;
  ...
  const pauseAutoPlay = useCallback(() => {
    clearAutoPlay();
    clearResume();
    // Desktop: stay paused; mobile: resume after idle
    if (isDesktop) return;
    resumeRef.current = setTimeout(() => {
      startAutoPlay();
    }, RESUME_AFTER_MS);
  }, [clearAutoPlay, clearResume, startAutoPlay, isDesktop]);
